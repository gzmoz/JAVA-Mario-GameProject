...
public class Handler { // tüm objeler arasýnda dolaþýr  ve bunlarý ayrý ayrý güncelleyip ekrana renderlar (dönüþtürmek).

    public LinkedList<Entity> entity = new LinkedList<>(); // bütün entitylerin objelerinin listesi-ne kadar entity olacaðý bilinmediði için
    ...
    public void render(Graphics g){
        for(Entity en:entity){ // ticktekiyle ayný þekilde gelen objenin Id'sine baðlý olarak looptan objeyi anlama ve iþleme (renderlama)
            en.render(g);
        }
        for(Tile ti:tile){
            ti.render(g);
    ...
    public void tick() { //update etmek??
        for(int i=0;i<entity.size();i++){ //
            entity.get(i).tick(); //get(i) linkedlist içindeki method ve kullanýcýnýn hangi objeyse onun Id'sini almasýný saðlar.
        }
        for(Tile ti:tile){
            ti.tick();
        }
    ...
    public void removeEntity(Entity en){// obje linkedliste çýkarma
        entity.remove(en);
    }
